...
def get_technology(event_name, source):
    ...
    if "agentic" in text or "agents" in text:
        return "Agentic AI"
    elif "generative" in text or "genai" in text or "gemini" in text:
        ...
        return "LLM"
    elif "hackathon" in text or "challenge" in text:
...
highest_scored_event = df.sort_values(by="AI Score", ascending=False).iloc[0]["Event"] if total_events > 0 else "N/A"
...
if page == "🏠 Dashboard":
    ...
    with h2:
        st.metric("Healthy Sources", healthy_count)

    with h3:
        st.metric("Warning Sources", warning_count)
    ...
    with d4:
        st.metric("Upcoming Soon", closing_soon_count)
...
st.caption("AI Events Radar v7.0 | Global Real-Time AI Event Intelligence Platform")