...
if page == "🏠 Dashboard":
...
elif page == "💼 AI Jobs Radar":

    st.markdown('<div class="section-title">💼 AI Jobs Radar</div>', unsafe_allow_html=True)

    jobs_df = df[
        df["Type"].astype(str).str.lower().str.contains("job", na=False) |
        df["Event"].astype(str).str.lower().str.contains("job", na=False) |
        df["Source"].astype(str).str.lower().str.contains("jobs", na=False) |
        df["Event"].astype(str).str.lower().str.contains("career", na=False)
    ]
    ...
    if not jobs_df.empty:
        ...
        with j1:
            st.metric("AI Job Sources", jobs_df["Source"].nunique())

        with j2:
            st.metric("AI Job Links", len(jobs_df))

        with j3:
            st.metric("High Priority", len(jobs_df[jobs_df["Priority"].astype(str).str.lower() == "high"]))

        with j4:
            st.metric("Free Access", len(jobs_df[jobs_df["Fee"].astype(str).str.lower() == "free"]))

        st.markdown("### 🔥 Recommended AI Job Tracks")
        show_table(jobs_df)

        st.markdown("### 🎯 Suggested Career Tracks")
        ...
        with c2:
            st.success("""
            **For ERP / Workday / SAP Professionals**
            - AI + ERP Consultant
        ...
        st.warning("""
        Use this page to quickly open AI-related job search links from platforms like LinkedIn, Naukri, Indeed, and other job sources.
        This is useful for private-company employees, support engineers, ERP consultants, testers, business analysts, and freshers who want to track AI career opportunities.
        """)

    else:
        st.warning("No AI Jobs Radar records found. Please run python fetch_events.py first.")
...
st.caption("AI Events Radar v8.5 | AI Jobs Radar Enabled | Global Real-Time AI Career Intelligence Platform")
