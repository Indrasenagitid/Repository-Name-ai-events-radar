...
st.sidebar.caption("v8.0 Product Experience Layer")
    ...
    if os.path.exists(source_status_path):
        ...
        for _, row in health_df.iterrows():
            ...
            if row["Status"] == "Healthy":
                st.success(
                    f"{row['Source']} is healthy with {row['Events Count']} event(s)."
                )

            elif row["Status"] == "Warning":
                st.warning(
                    f"{row['Source']} returned 0 events. Please review this source."
                )

            else:
                st.error(
                    f"{row['Source']} failed. Error: {row['Message']}"
                )
...
st.caption("AI Events Radar v8.0 | Global Real-Time AI Event Intelligence Platform | Product Experience Layer Enabled")